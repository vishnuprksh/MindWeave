...
app.post('/api/generate-map', async (req, res) => {
  ...
  try {
    ...
    logTiming('openrouter-error', { message: error instanceof Error ? error.message : 'unknown error' });
    return res.status(502).json({ error: 'Could not reach OpenRouter. Check the server network connection and API configuration.' });
  }
  try {
  ...
  } catch (error) { logTiming('request-error', { message: error instanceof Error ? error.message : 'unknown error' }); return res.status(500).json({ error: 'The map was generated, but could not be saved locally.' }); }
});
