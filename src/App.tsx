...
import { ArrowDownToLine, FileText, GitBranch, Lightbulb, Maximize2, MoreHorizontal, PanelLeftClose, PanelLeftOpen, RefreshCw, Search, Sparkles, Upload, Zap, GripVertical, LoaderCircle, Undo2, Redo2 } from 'lucide-react';
 ...
 function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`} style={{ width: sidebarCollapsed ? 0 : sidebarWidth }}>
      ...
      <div className="sidebar-footer">{user ? <><span className="status"><span className="green"/> {saveMessage || 'Signed in'}</span><button className="save-map" onClick={() => void saveMap()}>Save map</button><button className="icon-btn" onClick={() => void supabase?.auth.signOut()} title="Sign out"><RefreshCw size={15}/></button><div className="avatar">{(user.email?.slice(0, 2) || 'U').toUpperCase()}</div></> : <span className="status">Sign in to save maps</span>}</div>
    </aside>
