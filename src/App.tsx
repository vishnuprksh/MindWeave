...
function App() {
  ...
  const generate = async () => { if (!prompt.trim() || generating) return; setGenerating(true); setPromptError(''); try { const response = await fetch('/api/generate-map', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, markdown }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? 'Unable to generate the map.'); const nextMarkdown = data.markdown?.trim(); if (!nextMarkdown) throw new Error('The model returned an empty map.'); setMarkdown(nextMarkdown); setItems(parseMarkdown(nextMarkdown)); setSelected('n0'); } catch (error) { setPromptError(error instanceof Error ? error.message : 'Unable to generate the map.'); } finally { setGenerating(false); } };
