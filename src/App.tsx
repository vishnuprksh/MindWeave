...
import { ArrowDownToLine, FileText, GitBranch, Lightbulb, LogOut, Maximize2, MoreHorizontal, PanelLeftClose, PanelLeftOpen, Search, Sparkles, Upload, Zap, GripVertical, LoaderCircle, Undo2, Redo2 } from 'lucide-react';
 ...
 function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`} style={{ width: sidebarCollapsed ? 0 : sidebarWidth }}>
      ...
      <div className="sidebar-footer">{user ? <><span className="status"><span className="green"/> {saveMessage || 'Signed in'}</span><button className="save-map" onClick={() => void saveMap()}>Save map</button><button className="sign-out" onClick={() => void signOut()} disabled={authBusy}><LogOut size={14}/> {authBusy ? 'Signing out…' : 'Sign out'}</button><div className="avatar">{(user.email?.slice(0, 2) || 'U').toUpperCase()}</div></> : <span className="status">Sign in to save maps</span>}</div>
    </aside>
    <main className="main"><header className="topbar"><div className="breadcrumbs"><button className="icon-btn sidebar-toggle" onClick={() => setSidebarCollapsed(value => !value)} aria-label={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'} title={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{sidebarCollapsed ? <PanelLeftOpen size={17}/> : <PanelLeftClose size={17}/>}</button><span>My workspace</span><b>/</b><strong>Untitled map</strong></div><div className="top-actions"><div className="history-actions" aria-label="History"><button className="icon-btn" onClick={undo} disabled={!history.length} title="Undo"><Undo2 size={16}/></button><button className="icon-btn" onClick={redo} disabled={!future.length} title="Redo"><Redo2 size={16}/></button></div><div className="view-switcher" role="tablist" aria-label="Map view"><button className={view==='mindmap'?'active':''} onClick={()=>setView('mindmap')}>Mindmap</button><button className={view==='markdown'?'active':''} onClick={()=>setView('markdown')}>Markdown</button></div>{user ? <span className="user-email">{user.email}</span> : <button className="share" onClick={() => setAuthMessage('Open the authentication panel in the sidebar.') }><Zap size={15}/> Sign in</button>}<button className="icon-btn"><MoreHorizontal size={18}/></button></div></header><section className="workspace"><div className="canvas-head"><div><h1>{view === 'mindmap' ? 'Untitled map' : 'Source markdown'}</h1><p>{view === 'mindmap' ? <>Product launch strategy <span>•</span> Edited just now</> : <>Edit the outline to update the mindmap <span>•</span> Markdown source</>}</p></div>{view === 'mindmap' && <div className="canvas-tools"><div className="search"><Search size={15}/><input placeholder="Find in map" value={query} onChange={e=>setQuery(e.target.value)}/><kbd>⌘ F</kbd></div><button className="icon-btn"><Maximize2 size={16}/></button></div>}</div>{!user && <div className="auth-panel"><div className="eyebrow">{isSupabaseConfigured ? 'SAVE YOUR MINDMAPS' : 'SUPABASE SETUP REQUIRED'}</div>{isSupabaseConfigured ? <><div className="auth-tabs"><button className={authMode === 'sign-in' ? 'active' : ''} onClick={() => setAuthMode('sign-in')}>Sign in</button><button className={authMode === 'sign-up' ? 'active' : ''} onClick={() => setAuthMode('sign-up')}>Create account</button></div><div className="auth-fields"><input type="email" placeholder="Email" value={authEmail} onChange={e=>setAuthEmail(e.target.value)}/><input type="password" placeholder="Password" value={authPassword} onChange={e=>setAuthPassword(e.target.value)}/><button className="generate" onClick={() => void authenticate()} disabled={authBusy}>{authBusy ? 'Working…' : authMode === 'sign-in' ? 'Sign in' : 'Create account'}</button></div></> : <p>Add <code>VITE_SUPABASE_URL</code> and <code>VITE_SUPABASE_ANON_KEY</code> to <code>.env</code> to enable authentication.</p>}{authMessage && <div className="prompt-error">{authMessage}</div>}</div>}{view === 'mindmap' ? <div className="flow-wrap"><Mindmap items={filtered} selected={selected} select={select} edit={edit} moveItem={moveItem} zoom={zoom}/><div className="canvas-hint"><span className="key">TAB</span> child <span className="key">↵</span> sibling <span className="key">F2</span> edit <span className="key">DEL</span> delete</div><div className="zoom"><button onClick={()=>setZoom(z=>Math.min(1.3,z+.1))}>+</button><span>{Math.round(zoom*100)}%</span><button onClick={()=>setZoom(z=>Math.max(.7,z-.1))}>−</button></div></div> : <MarkdownEditor markdown={markdown} setMarkdown={updateMarkdown} onExport={() => { const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([markdown], {type:'text/markdown'})); a.download = 'mindmap.md'; a.click(); }} />}</section></main>
  </div>
}
