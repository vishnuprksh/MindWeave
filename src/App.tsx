...
import { ArrowDownToLine, FileText, GitBranch, Lightbulb, Maximize2, MoreHorizontal, RefreshCw, Search, Sparkles, Upload, Zap, GripVertical, LoaderCircle } from 'lucide-react';
...
function App() {
  const [activeFile, setActiveFile] = useState(mapFiles[0]?.name ?? 'mindmap.md'); const [markdown, setMarkdown] = useState(starter); const [items, setItems] = useState(() => parseMarkdown(starter)); const [selected, setSelected] = useState('n0'); const [query, setQuery] = useState(''); const [zoom, setZoom] = useState(1); const [view, setView] = useState<'mindmap'|'markdown'>('mindmap'); const [sidebarWidth, setSidebarWidth] = useState(286); const [prompt, setPrompt] = useState('Plan a product launch'); const [generating, setGenerating] = useState(false); const [promptError, setPromptError] = useState(''); const fileRef = useRef<HTMLInputElement>(null);
  ...
  const saveMap = async (value: string) => { const response = await fetch(`/api/maps/${encodeURIComponent(activeFile)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ markdown: value }) }); const data = await response.json() as { error?: string }; if (!response.ok) throw new Error(data.error ?? 'Could not save the active map.'); };
  const generate = async () => { if (!prompt.trim() || generating) return; setGenerating(true); setPromptError(''); try { const response = await fetch('/api/generate-map', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, markdown, fileName: activeFile }) }); const raw = await response.text(); let data: { error?: string; markdown?: string } = {}; if (raw.trim()) { try { data = JSON.parse(raw) as typeof data; } catch { throw new Error('The server returned an invalid response. Please try again.'); } } if (!response.ok) throw new Error(data.error ?? `Request failed (${response.status}).`); const nextMarkdown = data.markdown?.trim(); if (!nextMarkdown) throw new Error('The model returned an empty map.'); setMarkdown(nextMarkdown); setItems(parseMarkdown(nextMarkdown)); setSelected('n0'); } catch (error) { setPromptError(error instanceof Error ? error.message : 'Unable to update the map.'); } finally { setGenerating(false); } };
  useEffect(() => { const handler = (e: KeyboardEvent) => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') { e.preventDefault(); void generate(); } else if (e.key === 'Tab') { e.preventDefault(); addNode('child'); } else if (e.key === 'Enter' && document.activeElement?.tagName !== 'TEXTAREA') { e.preventDefault(); addNode('sibling'); } else if (e.key === 'Delete') remove(); }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler); });
  ...
  return <div className="app-shell">
    <aside className="sidebar" style={{ width: sidebarWidth }}>
      ...
      <div className="side-section prompt"><div className="eyebrow">PROMPT COMPOSER</div><textarea placeholder="Describe how to update this map..." value={prompt} onChange={e => setPrompt(e.target.value)} disabled={generating}/><button className="generate" onClick={() => void generate()} disabled={generating}>{generating ? <LoaderCircle className="spin" size={15}/> : <Sparkles size={15}/>} {generating ? 'Updating…' : 'Update active map'} <kbd>⌘ ↵</kbd></button>{promptError && <div className="prompt-error">{promptError}</div>}<div className="prompt-hint"><Lightbulb size={14}/> Updates {activeFile} directly</div></div>
