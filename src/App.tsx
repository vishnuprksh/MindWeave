...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar" style={{ width: sidebarWidth }}>
      ...
      <div className="side-section prompt"><div className="eyebrow">PROMPT COMPOSER</div><textarea placeholder="Describe what you want to map..." value={prompt} onChange={e => setPrompt(e.target.value)} disabled={generating}/><button className="generate" onClick={() => void generate()} disabled={generating}>{generating ? <LoaderCircle className="spin" size={15}/> : <Sparkles size={15}/>} {generating ? 'Generating…' : 'Generate map'} <kbd>⌘ ↵</kbd></button>{promptError && <div className="prompt-error">{promptError}</div>}<div className="prompt-hint"><Lightbulb size={14}/> Try: “Break down a complex topic”</div></div>
...
function MarkdownEditor({ markdown, setMarkdown, onExport }: { markdown: string; setMarkdown: (value: string) => void; onExport: () => void }) {
  return <div className="markdown-view"><div className="markdown-toolbar"><span className="markdown-file"><FileText size={16}/> mindmap.md</span><span className="markdown-meta">Markdown outline</span><button onClick={onExport}><ArrowDownToLine size={14}/> Export</button></div><textarea aria-label="Markdown source editor" spellCheck={false} value={markdown} onChange={e=>setMarkdown(e.target.value)} /></div>;
}
